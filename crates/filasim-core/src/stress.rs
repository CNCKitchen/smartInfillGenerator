...
pub enum FieldKind {
    ...
    SignedVonMises,
    /// Maximum principal stress σ₁ (MPa) — the tensile driver for brittle and
    /// layer-anisotropic materials, where von Mises (a shear criterion) under-
    /// reads a biaxial tensile state.
    S1,
    /// Minimum principal stress σ₃ (MPa).
    S3,
    Sxx,
    Syy,
    Szz,
                ...
                let v = match kind {
                    ...
                    _ => {
                        ...
                        match kind {
                            ...
                            FieldKind::Syz => syz,
                            FieldKind::Szx => szx,
                            FieldKind::S1 | FieldKind::S3 => {
                                let (s1, s3) = crate::orient::principal_range([
                                    sxx as f32, syy as f32, szz as f32, sxy as f32, syz as f32,
                                    szx as f32,
                                ]);
                                if matches!(kind, FieldKind::S1) {
                                    s1
                                } else {
                                    s3
                                }
