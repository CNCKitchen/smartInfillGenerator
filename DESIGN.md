...
## 25. Sample model on startup & pre-tessellated STEP cache (2026-07-28)

An empty viewport asks a first-time visitor to already own a CAD file; the
demo should instead be one click deep. On boot, filaSim now loads the CNC
Kitchen Hook (`CNCKTestHook.step`, repo root = source of truth) with a
working load case applied, staying on station 1 · Model — the normal
first-run flow (orient → loads → analyze), just with the rig pre-filled.

**Decisions.**

1. **Assets, not code.** `web/public/sample/` ships the original STEP bytes
   verbatim (the engine embed for project save needs them) plus a
   pre-tessellated `.mesh` container, both generated by `npm run sample:mesh`
   (`web/scripts/build-sample-mesh.mjs`) and committed. The script runs the
   EXACT app conversion under Node type stripping — the conversion moved
   from the import worker into `engine/stepConvert.ts` (shared, environment-
   agnostic) so there is one code path, not a port that can drift.
2. **The worker validates the cache, never trusts it.** The import worker
   accepts an optional cache blob with the request; it recomputes the STEP
   bytes' sha-256 and compares hash + meshStep `VERSION` + tessellation opts
   against the container (`engine/stepMeshCache.ts`: JSON header + typed-
   array blobs). Match ⇒ tessellation skipped (sample visible ~1 s after
   load, e2e-measured); any mismatch or decode error ⇒ silent fallback to a
   live import. A stale cache can cost time, never correctness. The payload
   carries a session-only `fromCache` flag, logged in the nerd log
   ("pre-tessellated cache") — also the e2e assertion hook.
3. **Canned loads bind by CAD-face entity id** (`sampleModel.ts`), the §18
   identity that is stable for these exact bytes: elastic support (100 N/mm³,
   a strap, not a bolted joint) on face 1787 — the big slot's far inner
   wall — and 250 N along −Y (straight away from the slot) on face 1826,
   the slim flat at the bottom of the small hook's eye (between the big eye
   cylinder and the bspline sector) — the purpose-built bearing surface the
   pull presses square onto. In-plane pull = the real hook-test load. If an
   id fails to resolve (inconsistent asset pair), the model ships bare
   rather than half-rigged.
4. **Race-safe and skippable.** One-shot latch + re-checks after each await:
   a user drop always wins; fetch failure (missing assets, offline dev)
   keeps the classic empty drop zone. Opt-out checkbox in the nerd log next
   to the disclaimer skip (`filasim-skip-sample` in localStorage).
5. **Not a `.filasim`.** The sample is a fresh import + two BCs, so users see
   the normal first-run flow (orient → loads → analyze), not a restored
   project with results baked in.
